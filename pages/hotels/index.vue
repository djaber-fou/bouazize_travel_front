    ...
    <div class="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 py-10 transition-colors duration-300">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="text-center">
                <div class="inline-flex items-center gap-2 px-3.5 py-1.5 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 rounded-full text-xs font-semibold uppercase tracking-wider mb-3 border border-blue-200 dark:border-blue-800">
                    <span class="w-2 h-2 rounded-full bg-blue-600 dark:bg-blue-400 animate-pulse"></span>
                    Netstorming Kalima XML Engine
                </div>
                ...
                <p class="mt-3 text-base sm:text-lg text-slate-600 dark:text-slate-400 max-w-2xl mx-auto">
                    Moteur de recherche et réservation séquentielle certifié Netstorming : <b>Disponibilité &rarr; Prébooking (Évaluation) &rarr; Confirmation Finale</b>.
                </p>
            </div>

            <!-- NETSTORMING CERTIFICATION SUITE PANEL -->
            <div class="bg-gradient-to-br from-slate-900 via-slate-900 to-indigo-950 rounded-2xl shadow-xl p-6 md:p-8 text-white border border-slate-800">
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                    <div>
                        <div class="flex flex-wrap items-center gap-3">
                            <h2 class="text-xl sm:text-2xl font-bold tracking-wide flex items-center gap-2">
                            ...
                            <span class="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-emerald-500/20 text-emerald-300 border border-emerald-500/30">
                                Flux Séquentiel 100% Automatisé
                            </span>
                        </div>
                        <p class="text-sm text-slate-400 mt-1">
                            Exécute la chaîne complète obligatoire : <b>Availability &rarr; Prebooking (Evaluation) &rarr; Booking &rarr; Track</b>.
                        </p>
                    </div>
                    ...
                    <button 
                        ...
                        {{ certLoading ? 'Exécution du test...' : 'Lancer le Test Complet (1-Clic)' }}
                    </button>
                </div>
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            ...
                            <div>
                                <div class="flex justify-between items-center mb-2">
                                    ...
                                    <button @click="copyReference(certResult.steps[activeTab].request)" class="text-xs text-indigo-400 hover:text-indigo-300 cursor-pointer">
                                        Copier
                                    </button>
                                </div>
                            ...
                            <div>
                                <div class="flex justify-between items-center mb-2">
                                    ...
                                    <button @click="copyReference(certResult.steps[activeTab].response)" class="text-xs text-indigo-400 hover:text-indigo-300 cursor-pointer">
                                        Copier
                                    </button>
                                </div>
            ...
            <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 p-6 md:p-8">
                ...
                <div class="flex flex-wrap items-center gap-2 mb-6 text-xs">
                    <span class="text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider">Villes rapides :</span>
                    <button type="button" @click="form.city_code = 'DXB'; form.hotel_ids = ''; searchHotels()" class="px-3 py-1.5 rounded-full bg-slate-100 dark:bg-slate-800 hover:bg-blue-50 dark:hover:bg-blue-900/40 hover:text-blue-700 dark:hover:text-blue-300 text-slate-700 dark:text-slate-300 font-semibold border border-slate-200 dark:border-slate-700 transition-colors cursor-pointer">
                        🇦🇪 Dubaï (DXB)
                    </button>
                    <button type="button" @click="form.city_code = 'PAR'; form.hotel_ids = ''; searchHotels()" class="px-3 py-1.5 rounded-full bg-slate-100 dark:bg-slate-800 hover:bg-blue-50 dark:hover:bg-blue-900/40 hover:text-blue-700 dark:hover:text-blue-300 text-slate-700 dark:text-slate-300 font-semibold border border-slate-200 dark:border-slate-700 transition-colors cursor-pointer">
                        🇫🇷 Paris (PAR)
                    </button>
                    <button type="button" @click="form.city_code = 'VCEI'; form.hotel_ids = ''; searchHotels()" class="px-3 py-1.5 rounded-full bg-slate-100 dark:bg-slate-800 hover:bg-blue-50 dark:hover:bg-blue-900/40 hover:text-blue-700 dark:hover:text-blue-300 text-slate-700 dark:text-slate-300 font-semibold border border-slate-200 dark:border-slate-700 transition-colors cursor-pointer">
                        🇮🇹 Venise (VCEI)
                    </button>
                    <button type="button" @click="form.city_code = 'IST'; form.hotel_ids = ''; searchHotels()" class="px-3 py-1.5 rounded-full bg-slate-100 dark:bg-slate-800 hover:bg-blue-50 dark:hover:bg-blue-900/40 hover:text-blue-700 dark:hover:text-blue-300 text-slate-700 dark:text-slate-300 font-semibold border border-slate-200 dark:border-slate-700 transition-colors cursor-pointer">
                        🇹🇷 Istanbul (IST)
                    </button>
                    <button type="button" @click="form.city_code = 'LON'; form.hotel_ids = ''; searchHotels()" class="px-3 py-1.5 rounded-full bg-slate-100 dark:bg-slate-800 hover:bg-blue-50 dark:hover:bg-blue-900/40 hover:text-blue-700 dark:hover:text-blue-300 text-slate-700 dark:text-slate-300 font-semibold border border-slate-200 dark:border-slate-700 transition-colors cursor-pointer">
                        🇬🇧 Londres (LON)
                    </button>
                    <button type="button" @click="form.hotel_ids = '25846, 12296'; form.city_code = ''; searchHotels()" class="px-3 py-1.5 rounded-full bg-indigo-50 dark:bg-indigo-950/50 text-indigo-700 dark:text-indigo-300 font-semibold border border-indigo-200 dark:border-indigo-800 hover:bg-indigo-100 transition-colors cursor-pointer">
