    ...
    <div class="w-full space-y-4 pb-4">
        ...
        class:'cursor-pointer',
            ...
            <template #body>
                <div class="flex flex-col gap-5">
                    <div class="flex flex-col  gap-2">
                        <p class="text-secondary">Omra:</p>
                        <div class="flex items-center gap-3">
                            <UAvatar :src="order?.Omra?.country?.flag" size="sm"/>
                            <div>
                                {{ order?.Omra?.country?.name }}
                            </div>
                        </div>
                    </div>
                    <div class="flex flex-col  gap-2">
                        <p class="text-secondary">Offre:</p>
                        <div>{{ order?.Omra?.name }}</div>
                    </div>
                    <div class="flex flex-col  gap-2">
...
const columns = [
    ...
    },
    {
        ...
        header: 'Pay',
        cell:({row})=>{
            return h('div',undefined,row.original.Omra.country.name)
        }
    },
    {
        accessorKey: 'Omra',
        header: 'Offre',
        cell:({row})=>{
            return h('div',undefined,row.original.Omra.name)
        }
    },
    {
        accessorKey: 'price',
...
const getOrders = async (page=1)=>{
    loading.value = true
    sendApi(`/admin/omra/orders?page=${page}&per_page=10&search=${search.value}&status=${orderStatusValue.value}`,null,'GET').then(response=>{
...
const getOrder = async (id)=>{
    loading.value = true
    sendApi(`/admin/omra/orders/${id}`,null,'GET').then(response=>{
        console.log(response)
...
const acceptOrder = async ()=>{
    ...
    sendApi(`/admin/omra/orders/${order.value.id}/accept`,formData,'POST').then(()=>{
...
const markAsPaid = async ()=>{
    ...
    sendApi(`/admin/omra/orders/${order.value.id}/paid`,null,'PUT').then(()=>{
        getOrder(order.value.id)
        getOrders(pagination.value.pageIndex + 1)
        loadingAction.value = null
    })
...
const rejectOrder = async ()=>{
    ...
    formData.append('_method','PUT')
    sendApi(`/admin/omra/orders/${order.value.id}/reject`,formData,'POST').then(()=>{
        getOrder(order.value.id)
        getOrders(pagination.value.pageIndex + 1)
        openAccept.value = false
        orderAction.value = null
...
const deleteOrder = async (id)=>{
    sendApi(`/admin/omra/orders/${id}/delete`,null,'DELETE').then(()=>{
        open.value = false
        getOrders(pagination.value.pageIndex + 1)
    })
}
...
const getInvoice = async(id)=>{
    const response = sendApi(`/admin/omra/orders/${id}/invoice`,null,'GET').then((response)=>{
...
const createInvoice = async()=>{
    ...
    sendApi(`/admin/omra/orders/${order.value.id}/create-invoice`,{bank_info:bank_info.value},'POST').then(()=>{
        order.value.id = null
...
const downloadZip = async (user,id)=> {
    ...
    const response = await useFetch(import.meta.env.VITE_BASE_URL+`/admin/omra/orders/${id}/download`,{
        method:'GET',
        headers:{
